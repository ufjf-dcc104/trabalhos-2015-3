...
function Sprite(){
	...
	this.moverNoGrid= function(grid,dt,g){
		...
		if(grid[this.gy+1][this.gx]==1 || grid[this.gy+1][this.gx]==2 || grid[this.gy+1][this.gx]==6){
			var dy = Math.min(
				this.vy*dt,
			...
			var dy = Math.max(
				this.vy*dt,
				(this.gy)*this.altura-(this.y-this.altura/2)
			);
			this.y += Math.ceil(dy);
		...
		if(this.x+this.largura/2>this.largura*60) this.x = this.largura*50-this.largura/2;	
	...
	this.desenhar = function(ctx, jogo){//contexto e informações do jogo (vidas, pontos...)
		...
		if(this.sobBloco){///animação: parado sob um bloco que se movimenta
			this.p=1;
		}
		...
		if(jogo.fimdejogo){//desenha o boneco "morto" do sprite
			ctx.drawImage(this.img, ///1,1,16,16,/// corte: pos inicial x, y tamanho do corte l, a
						5*this.largura, this.altura, this.largura, this.altura,/// corte: pos inicial x, y tamanho do corte l, a
						//0, 0, 16, 16);///imagem: pos x,y tamanho l,a
						-this.largura/2, -this.altura/2, this.largura, this.altura);///imagem: pos x,y tamanho l,a
		}else{//desenho normal
			ctx.drawImage(this.img, ///1,1,16,16,/// corte: pos inicial x, y tamanho do corte l, a
